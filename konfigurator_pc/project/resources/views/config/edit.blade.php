...
@section('body')
    <form method="POST" action="{{ route('config.update', ['config'=>$config] ) }}" class="bg-white flex-col justify-center m-2 p-2 shadow">
        @csrf
        @method('PUT')
        ...
        <div class="flex justify-center mb-3 pt-0">
            <textarea name="desc" placeholder="Description" required :value="$config->desc"
        ...
        <div class="flex justify-center">
            <button id="createBtn" disabled
                    class="m-6 w-1/2 h-14 px-6 text-indigo-100 transition-colors duration-150 bg-gray-400 rounded-lg focus:shadow-outline">{{ __('Update PC configuration') }}</button>
        </div>
    </form>
